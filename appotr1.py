...
EXPECTED_COLUMNS = {
    ...
    "GPMP PPT": ["GPMP PPT", "PPT"],
    "GPMP OTR1": ["GPMP OTR1", "OTR1"],
}
...
def compute_ipmp(df: pd.DataFrame) -> pd.DataFrame:
    ...
    needed = ["Mata Pelajaran", "Bil. Ambil", "GPMP PPT", "GPMP OTR1"]
    for c in needed:
        if c not in df.columns:
            raise ValueError(f"Kolum '{c}' tiada dalam data.")
    # Coerce numerics
    for c in ["Bil. Daftar", "Bil. Ambil", "GPMP PPT", "GPMP OTR1"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0)

    # Calculations
    df["Beza GPMP (OTR1 - PPT)"] = df["GPMP OTR1"] - df["GPMP PPT"]
...
st.sidebar.markdown("---")
st.sidebar.caption("Kolum diperlukan: Mata Pelajaran, Bil. Ambil, GPMP PPT, GPMP OTR1. (Bil. Daftar opsyenal).")
...
long_df = df_result.melt(id_vars=["Mata Pelajaran"], value_vars=["GPMP PPT", "GPMP OTR1"],
                         var_name="Jenis", value_name="GPMP")
fig2 = px.line(long_df, x="Mata Pelajaran", y="GPMP", color="Jenis",
               markers=True, title="GPMP OTR1 vs PPT Mengikut Subjek")
